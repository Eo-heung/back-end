...
public interface FriendRepository extends JpaRepository<Friend, Long> {
    ...
    @Query("SELECT f FROM Friend f WHERE (f.fromUser = :name OR f.toUser = :name) AND f.status = false ")
    List<Friend> findFriendsByFromUserOrToUserAndStatusFalse(@Param("name") String name);
